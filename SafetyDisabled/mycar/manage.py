...
# from donkeycar.keras import KerasSafetyRNN
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    if False and cfg.AUTO_RECORD_ON_THROTTLE:
    ...
    if model_path:
        ...
        print("searching for", model_type)
        kl = dk.utils.get_model_by_type(model_type, cfg)
        # kl = KerasSafetyRNN()
        print("loaded Model type:", kl, type(kl))
        ...
        # rospy.Subscriber("/odom_combined", PoseWithCovarianceStamped, lambda msg: kl.receive_data(msg))
        # server = RosServer(lambda _: rospy.spin())
        # server.start()
    ...
    class DriveMode:
        @staticmethod
        def convert_to_cm(x, y) -> "tuple[float, float]":
            return (
                (220. - y * 220. / 2) - 12., #THIS IS THE X AXIS NOW
                (x * 220. / 2.) + 12.       #THIS IS THE Y AXIS NOW
            )
        ...
        def __init__(self):
            ...
            self.state = (0., 0., 1., 0.)
            rospy.init_node('listener', anonymous=True)
            rospy.Subscriber("/odom_combined", PoseWithCovarianceStamped, lambda msg: self.receive_data(msg))
            # server = RosServer(lambda _: rospy.spin())
            # server.start()
        
        def receive_data(self, msg: PoseWithCovarianceStamped):
            ori = msg.pose.pose.orientation
            _, _, z, w = ori.x, ori.y, ori.z, ori.w
            sinus = 2 * w * z
            cosinus = 1 - 2 * z * z

            pos = msg.pose.pose.position
            x, y = self.convert_to_cm(pos.x, pos.y)
            self.state = (x, y, cosinus, sinus)
        ...
        def run(self, mode,
            ...
            #print(pilot_angle)
            if mode == 'user':
                recovery_action = self.safety_data.get_recovery_action(self.state)
                x, y = self.state[:2]
                ...
                # print("The user input:", user_angle, user_throttle)
                return user_angle, user_throttle
                # self.count += 1
                # if not self.done and self.count > 120:
                #     self.done = True
                #     self.count = 0
                #     print(self.done)
                # if self.done and self.count < 3:
                #     return -1., 0.
                # elif self.done and self.count < 12:
                #     return -1., 0.
                # #else:
                # #    return 0., 0.
